...
sendTezzies config from fromName dests sign = do
  ...
  let txns = fmap (\((dest, amount), counter) -> A.toJSON $ M.fromList [("kind" :: T.Text, A.String "transaction"), ("amount", A.toJSON amount), ("source", A.toJSON from),
                ("destination", A.String dest), ("storage_limit", A.String "0"), ("gas_limit", A.String "10300"), ("fee", A.toJSON $ Tezzies $ 0.001420), ("counter", A.toJSON $ P.show counter)]) (P.zip dests [currentCounter + 1 ..])
  ...
  P.print (P.length dests, P.length succeeded, contents)
